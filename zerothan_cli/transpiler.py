...
class PythonASTAnalyzer(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        # Default to public if no explicit decorators and not hidden
        if not is_public and not is_view and not node.name.startswith('_'):
            is_public = True
            console.print(f"[cyan]Assuming public function: {node.name}[/cyan]")
